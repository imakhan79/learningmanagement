...
DROP POLICY IF EXISTS "exam_templates_select" ON exam_templates;
...
DROP POLICY IF EXISTS "exam_templates_insert" ON exam_templates;
...
DROP POLICY IF EXISTS "exam_templates_update" ON exam_templates;
CREATE POLICY "exam_templates_update" ON exam_templates FOR UPDATE
  USING (EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.role IN ('admin','professor')))
  WITH CHECK (EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.role IN ('admin','professor')));

DROP POLICY IF EXISTS "exam_templates_delete" ON exam_templates;
