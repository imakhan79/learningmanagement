...
DROP POLICY IF EXISTS "Students can view their own submissions" ON worksheet_submissions;
CREATE POLICY "Students can view their own submissions"
    ON worksheet_submissions FOR SELECT
    USING (auth.uid() = student_id);

DROP POLICY IF EXISTS "Students can insert their own submissions" ON worksheet_submissions;
CREATE POLICY "Students can insert their own submissions"
    ON worksheet_submissions FOR INSERT
    WITH CHECK (auth.uid() = student_id);

DROP POLICY IF EXISTS "Professors can view submissions for their courses" ON worksheet_submissions;
CREATE POLICY "Professors can view submissions for their courses"
    ON worksheet_submissions FOR SELECT
    USING (
        EXISTS (
...
DROP POLICY IF EXISTS "Admins can view all submissions" ON worksheet_submissions;
...
-- 2. Exams Type (Quiz support)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema='public' AND table_name='exams' AND column_name='type'
    ) THEN
        ALTER TABLE exams ADD COLUMN type TEXT NOT NULL DEFAULT 'exam' CHECK (type IN ('exam', 'quiz'));
    END IF;
END $$;
